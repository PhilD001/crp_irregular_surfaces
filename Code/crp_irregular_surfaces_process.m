...
% This code computes continuous relative phase metrics for the uneven walking surface
% dataset. 

% INSTRUCTIONS: See README.md for detailed running instructions

% Last updated by Philippe C. Dixon Feb 3 2021
...
for g = 1:length(group)
    ...
    for s = 1:length(surface)
        ...
        for p = 1:length(participant)
            ...
            fld_gsp = [fld_data_proc, filesep, group{g}, filesep, participant{p},...
                       filesep, surface{s}];
            fl = engine('fld', fld_gsp, 'extension', 'zoo');
            
            disp(' ')
            disp(['extracting data from ', num2str(length(fl)), ...
                  ' trials for subject ', participant{p}, ' on surface ', ...
                  surface{s}, ' ...'])

            % initialize stk for each CRP metric by group/surface/participant -------
            ...
            for f = 1:length(fl)
                ...
                % Checks which limb side is associated with middle step (apex) ------
                ApexFoot=data.zoosystem.CompInfo.ApexFoot;
                                
                % Extracts Stance / Swing indices based on existing gait events -----
                ...
                SACR_mag = sqrt(SACR(:,1).* SACR(:,1) + SACR(:,2).* SACR(:,2) + ...
                                SACR(:,3).* SACR(:,3));
                SACR_mag = SACR_mag/1000;                             % convert to m
                time = length(SACR_mag)/data.zoosystem.Video.Freq;    % freq to time
                vel_stk(f) = abs((SACR_mag(end)-SACR_mag(1))/time);   % velocity m/s
                ...
                for l = 1:length(limb)
                    ...
                    % Hip, knee, ankle phase angle padded to nearest event on either side ------
...
subjects = {'OA03','OA06','OA08','OA10','OA11','OA12','OA14','OA15','OA17','OA18',...
            'OA19','OA20','OA21','OA22','OA23','OA25','OA26','YA01','YA02','YA03',...
            'YA04','YA05','YA06','YA07','YA08','YA09','YA10','YA11','YA12','YA13',...
            'YA16','YA17','YA18','YA19','YA20'};
cons = {'Old\Flat', 'Old\Uneven', 'Young\Flat', 'Young\Uneven'};
lcl_evts = {'IC', 'LR', 'MS', 'TS', 'PSw', 'ISw', 'MSw', 'TSw', 'velocity'};
chns = {'RightKHStance_MARP','RightKHSwing_MARP', 'RightAKStance_MARP',...
        'RightAKSwing_MARP', 'RightKHStance_DP', 'RightKHSwing_DP', ...
        'RightAKStance_DP', 'RightAKSwing_DP'};   
